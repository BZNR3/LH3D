...
class LSSFPN(nn.Module):
    ...
    def _forward_single_sweep(self,
        ...
        # ---- Compute m_depth_img: [B, D] (hard-mode: per-pixel top-1 histogram; summed across cameras) ----
        if return_depth_profile:
            ...
            if depth_profile_mode == "hard":
                ...
                m_flat.scatter_add_(1, idx_flat, w)  # Accumulate weights by top-1 depth bin
                m_img = m_flat.view(B, num_cams, D).sum(dim=1)  # Sum across cameras -> [B, D]
            else:
                # soft-mode: spatial average probability (can apply power sharpening)
                m_cam = height.mean(dim=(2, 3))  # [B*num_cams, D]
                m_img = m_cam.view(B, num_cams, D).mean(dim=1)  # [B, D]

            m_depth_img = m_img / (m_img.sum(dim=1, keepdim=True).clamp_min(1e-6))  # L1 normalize
        else:
            m_depth_img = None
        ...
        if is_return_height and return_bin_entropy:
            return (feature_map.contiguous(), height, H_img)  # <<< return triple
        if is_return_height:
            return feature_map.contiguous(), height
        if return_bin_entropy:
            return feature_map.contiguous(), H_img  # <<< return pair
    ...
    def forward(
            ...
            return_bin_entropy: bool = False,
            return_depth_profile: bool = False,  # <<< new parameter
            depth_profile_mode: str = "hard",  # <<< new parameter
            depth_profile_weight: str = "none",  # <<< new parameter
    ):
        # Run key-frame only (index 0)
        out = self._forward_single_sweep(
            ...
            return_depth_profile=return_depth_profile,  # <<< pass-through
            depth_profile_mode=depth_profile_mode,  # <<< pass-through
            depth_profile_weight=depth_profile_weight,  # <<< pass-through
        )
        # Do not split or modify the return here; pass through _forward_single_sweep's output as-is
        return out
