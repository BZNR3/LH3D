...
data_root = "/data/dair-v2x-i/Full_Dataset"
...
class BEVHeightLightningModel(LightningModule):
    ...
    def __init__(self,
        ...
        mkdir_or_exist(default_root_dir)
...
def main(args: Namespace) -> None:
    ...
    checkpoint_callback = ModelCheckpoint(dirpath='/path/to/checkpoint', filename='{epoch}', every_n_epochs=5, save_last=True, save_top_k=-1)
    trainer = pl.Trainer.from_argparse_args(args, callbacks=[checkpoint_callback])

    if args.evaluate:
        if os.path.isfile(args.ckpt_path):
            # Directly test a single checkpoint file
            trainer.test(model, ckpt_path=args.ckpt_path)
        else:
            # Evaluate all checkpoints in the folder
        ...
        backup_codebase(os.path.join('/<storage>/<user>/<project>/outputs/<experiment>', 'backup'))
        trainer.fit(model)
...
def run_cli():
    ...
    parent_parser.add_argument('-b', '--batch_size_per_device',default=8, type=int)
    parent_parser.add_argument('--seed',
                               type=int,
                               default=0,
                               help='seed for initializing training.')
    parent_parser.add_argument('--ckpt_path', default='/path/to/checkpoint', type=str)
