...
BEVHEIGHT_ROOT = "/path/to/BEVHeight"
...
data_root = "/path/to/data/Rope3D_data"
gt_label_path = "/path/to/data/rope3d-kitti/training/label_2"
...
class BEVHeightLightningModel(LightningModule):
    ...
    def _build_train_dataset(self):
        ds = NuscMVDetDataset(
            ...
            info_path=os.path.join(self.data_root, 'rope3d_12hz_infos_hom_train.pkl'),
            is_train=True,
    ...
    def configure_optimizers(self):
        # Infer effective world size for correct LR scaling
        ...
        if world is None:
            if isinstance(self.gpus, int):
                ...
                dv = getattr(self, "_devices_arg", None)
                try:
    ...
    def train_dataloader(self):
        ...
        dataset = self._train_dataset if self._train_dataset is not None else self._build_train_dataset()
        if self._labeled_indices is not None:
            train_dataset = Subset(dataset, self._labeled_indices)
        else:
    ...
    def val_dataloader(self):
        val_dataset = NuscMVDetDataset(
            ...
            classes=self.class_names,
            data_root=self.data_root,
            info_path=os.path.join(self.data_root, 'rope3d_12hz_infos_hom_val.pkl'),
            is_train=False,
            img_conf=self.img_conf,
            num_sweeps=self.num_sweeps,
            sweep_idxes=self.sweep_idxes,
...
def main(args: Namespace) -> None:
    ...
    if getattr(args, 'al_enabled', False):
        print("[AL] Active Learning enabled for Rope3D.")
...
def run_cli():
    ...
    parent_parser.add_argument('--al_enabled',
                               action='store_true',
                               default=True,
                               help='Enable active learning loop')
    parent_parser.add_argument('--al_method',
                               type=str,
                               default='PPAL',
                               help='Active learning method name') # entropy lh3d
    parent_parser.add_argument('--al_init_size',
                               type=int,
                               default=500,
                               help='Initial labeled set size') # 500, 31506
    parent_parser.add_argument('--al_query_size',
                               type=int,
                               default=600,
                               help='Images to add per AL round') # 120 600
    parent_parser.add_argument('--al_rounds',
                               type=int,
                               default=10,
                               help='Number of AL rounds')
    parent_parser.add_argument('--al_epochs_per_round',
                               type=int,
                               default=5,
                               help='Epochs per AL round')
    parent_parser.add_argument('--al_pool_seed',
                               type=int,
                               default=0,
                               help='Random seed for pool shuffling')
    parent_parser.add_argument('--al_max_objects',
                               type=int,
                               default=170000,
                               help='Optional object budget for labeling') #32000 170000
    ...
    parser.set_defaults(
        ...
        precision=32,
        amp_backend='native',
        default_root_dir='outputs/rope3d_active')
